...
DYOR HQ is a static-generated investment research website. It produces HTML reports for individual equity tickers, stored in `projects/dyorhq/reports/`, registered in `reports/index.json`, validated at build time, and deployed to Netlify at dyorhq.ai via GitHub push to the `dyor-v4-work` branch.
...
| 9 | Create `.github/workflows/build-and-deploy.yml` | Push to `dyor-v4-work` triggers build + health check |
| 10 | Run full build and push to `dyor-v4-work` | Netlify deploys; health check passes; live count matches canonical |
