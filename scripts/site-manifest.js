...
const VALID_RECOMMENDATIONS = new Set(['BUY', 'OPPORTUNISTIC BUY', 'SPECULATIVE BUY', 'REDUCE']);
...
function validateProject() {
  ...
  canonicalIndex.forEach((entry, index) => {
    ...
    if (entry.recommendation && !VALID_RECOMMENDATIONS.has(entry.recommendation)) {
      issues.push(`${label}: recommendation "${entry.recommendation}" is not a valid tier (BUY, OPPORTUNISTIC BUY, SPECULATIVE BUY, REDUCE)`);
    }
